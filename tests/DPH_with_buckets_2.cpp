...
SCENARIO("DPH_with_buckets_2's basic functions work", "[hashtable]") {
	GIVEN("A DPH_with_buckets_2") {
		hashtable::DPH_with_buckets_2<unsigned int, unsigned int> m(100);
		const size_t n = 96;
...
SCENARIO("DPH_with_buckets_2 with string keys or values", "[hashtable]") {
	GIVEN("a DPH_with_buckets_2 with string keys and int values") {
		hashtable::DPH_with_buckets_2<std::string, int> m(100);
		WHEN("We insert keys") {
			m["foo"] = 1;
			m["bar"] = 2;
	...
	GIVEN("a DPH_with_buckets_2 with string keys and values") {
		hashtable::DPH_with_buckets_2<std::string, std::string> m(100);
		WHEN("We insert keys") {
			m["foo"] = "oof";
			m["bar"] = "baz";
			THEN("We can retrieve them again") {
...
SCENARIO("DPH_with_buckets_2 0/bucket size hashing failure", "[hashtable]") {
	GIVEN("A DPH_with_buckets_2") {
		hashtable::DPH_with_buckets_2<int, std::string> m(97);
		m[0] = "Null";
		m[3] = "Drei";
		m[55] = "Fünfundfünfzig";
...
SCENARIO("DPH_with_buckets_2 rehash counting", "[hashtable]") {
	GIVEN("A DPH_with_buckets_2 multiple times") {
		size_t hashtableAmount = 10;
		size_t hashtableSize = 100;
		size_t elementAmount = hashtableSize * 2;
		for (size_t h = 0; h < hashtableAmount; h++) {
			hashtable::DPH_with_buckets_2<int, int> m(hashtableSize);
