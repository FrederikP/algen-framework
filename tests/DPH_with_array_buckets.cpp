...
SCENARIO("DPH_with_array_buckets with string keys or values", "[hashtable]") {
	GIVEN("an DPH_with_array_buckets with string keys and int values") {
		hashtable::DPH_with_array_buckets<std::string, int> m(100, 10);
		WHEN("We insert keys") {
			m["foo"] = 1;
			m["bar"] = 2;
	...
	GIVEN("an DPH_with_array_buckets with string keys and values") {
		hashtable::DPH_with_array_buckets<std::string, std::string> m(100, 10);
		WHEN("We insert keys") {
			m["foo"] = "oof";
			m["bar"] = "baz";
			THEN("We can retrieve them again") {
